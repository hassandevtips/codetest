   ...
   public function store($user, $data)
{
    ...
    if (!isset($data['from_language_id'])) {
        return $this->errorResponse("from_language_id");
    }

    if ($data['immediate'] == 'no' && (!$this->validateDueFields($data))) {
        return $this->errorResponse("due_date");
    }

    $this->setCustomerPhoneAndPhysicalTypes($data);
...
private function validateDueFields($data)
{
    return !(
        empty($data['due_date']) ||
        empty($data['due_time']) ||
...
private function setCustomerPhoneAndPhysicalTypes(&$data)
{
...
{
    $gender = $this->mapGender($data['job_for']);
    $certified = $this->mapCertified($data['job_for']);

    $data['gender'] = $gender;
    $data['certified'] = $certified;
}

private function mapGender($jobForArray)
{
    if (in_array('male', $jobForArray)) {
        return 'male';
    } elseif (in_array('female', $jobForArray)) {
        return 'female';
    }
...
private function mapCertified($jobForArray)
{
    if (in_array('normal', $jobForArray) && in_array('certified', $jobForArray)) {
        return 'both';
    } elseif (in_array('normal', $jobForArray) && in_array('certified_in_law', $jobForArray)) {
        return 'n_law';
    } elseif (in_array('normal', $jobForArray) && in_array('certified_in_helth', $jobForArray)) {
        return 'n_health';
    } elseif (in_array('normal', $jobForArray)) {
        return 'normal';
    } elseif (in_array('certified', $jobForArray)) {
        return 'yes';
    } elseif (in_array('certified_in_law', $jobForArray)) {
        return 'law';
    } elseif (in_array('certified_in_helth', $jobForArray)) {
        return 'health';
    }
...
private function errorResponse($fieldName)
{
    $response['status'] = 'fail';
   ...
   public function storeJobEmail($data)
{
    ...
    if ($job) {
        ...
        $this->sendJobCreatedEmail($email, $name, $subject, $sendData);
    ...
    // Handle the case where the job was not found.
    $response = ['status' => 'fail', 'message' => 'Job not found'];
    return $response;
}

private function sendJobCreatedEmail($email, $name, $subject, $sendData)
{
     ...
     */
   /**
 * Convert a job object to data for sending Push.
 *
 * @param Job $job
 * @return array
 */
public function jobToData(Job $job)
{
    $data = [
        ...
        'certified' => $this->getCertifiedLabel($job->certified),
        'due' => $this->formatDueDate($job->due),
        'job_type' => $job->job_type,
        'customer_phone_type' => $this->getPhoneTypeLabel($job->customer_phone_type),
        'customer_physical_type' => $this->getPhysicalTypeLabel($job->customer_physical_type),
...
/**
 * Handle the end of a job session.
 *
...
public function jobEnd(array $post_data)
{
    ...
    if (!$job) {
        return; // Handle the case where the job is not found.
    }

    $sessionTime = $this->calculateSessionTime($job->due, $completeddate);
    $user = $job->user()->first();
    $email = $this->getUserEmail($user, $job->user_email);
    $name = $user->name;
    $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
    $sessionTimeText = $this->formatSessionTime($sessionTime);
    $data = [
        'user' => $user,
        'job' => $job,
